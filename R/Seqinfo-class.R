...
.make_Seqinfo_from_genome <- function(genome)
{
    ...
    if (genome %in% NCBI_assemblies[ , "assembly"] ||
        ...
        return(GenomeInfoDb::getChromInfoFromNCBI(genome, as.Seqinfo=TRUE))
    UCSC_genomes <- GenomeInfoDb::registered_UCSC_genomes()
    if (genome %in% UCSC_genomes[ , "genome"])
        return(GenomeInfoDb::getChromInfoFromUCSC(genome, as.Seqinfo=TRUE))
    stop(wmsg("\"", genome, "\" is not a registered NCBI assembly or ",
              "UCSC genome (use GenomeInfoDb::registered_NCBI_assemblies() ",
              "or GenomeInfoDb::registered_UCSC_genomes() to list the NCBI ",
              "or UCSC assemblies/genomes currently registered in the ",
